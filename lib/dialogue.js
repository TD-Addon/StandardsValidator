...
function intentionallyLeftBlank(record) {
	if(record.result) {
...
function needsNoLore(record, topic, mode) {
	if(mode === 'TD') {
		if(record.data.dialogue_type === 'Greeting' || record.speaker_faction) {
			return false;
		}
	}
	return !(record.data.speaker_rank >= HIGH_RANK) // High-level faction dialogue
...
module.exports = {
	onInfo(record, topic, mode) {
		...
		if(!record.text?.length) {
			if(!['Journal', 'Voice'].includes(record.data?.dialogue_type) && !intentionallyLeftBlank(record)) {
			...
			if(/(^|\s)an?\s+%PC/i.test(record.text)) {
				console.log(record.type, record.info_id, 'in topic', topic.id, 'contains an indefinite article followed by a PC variable');
			}
		}
		record.filters?.forEach(filter => {
			if(filter.filter_type === 'Dead' && filter.filter_comparison === 'Equal' && filter.value?.Integer) {
				console.log(record.type, record.info_id, 'in topic', topic.id, 'checks for Dead =', filter.value.Integer);
			}
		});
		if(record.speaker_id) {
			const isPlayer = record.speaker_id.toLowerCase() === 'player';
			if(!isPlayer) {
			...
			record.filters?.forEach(filter => {
				if(['Local', 'NotLocal'].includes(filter.filter_type)) {
					...
					if(filter.filter_type === 'NotFaction') {
			...
			if(mode === 'TD' && record.speaker_rank) {
				const race = record.speaker_rank.toLowerCase();
				if(PREFIXES.some(p => race.startsWith(p))) {
					return;
				}
			}
