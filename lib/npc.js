...
const { isAutoCalc, isFemale, isDead } = require('./util');
...
module.exports = {
	onRecord(record, mode) {
		...
		if(record.type === 'Npc') {
			...
			if(!isDead(record) && record.ai_data?.fight >= HOSTILE && record.ai_data.alarm >= BOUNTY_ALARM) {
				console.log(record.type, record.id, 'reports crimes despite having', record.ai_data.fight, 'fight');
			}
			if(!isDead(record) && record.ai_data?.alarm < BOUNTY_ALARM && record.class?.toLowerCase() === 'guard') {
