	...
	onRecord(record, topic, mode) {
		if(record.type === 'Info') {
			if(record.speaker_class) {
				...
				if(replacementClass) {
					console.log(record.type, record.id, 'in topic', topic.id, 'has a', record.id, 'filter');
				}
			}
			record.filters?.forEach(filter => {
				if(filter.filter_type === 'NotClass') {
					...
					if(replacementClass) {
						console.log(record.type, record.id, 'in topic', topic.id, 'has a Not Class', filter.id, 'filter');
					}
				}
			});
