...
const { isInterior, getCellName, CELL_SIZE, getCellGrid, hasWater } = require('./util');
const MIN_Z = -32000;
const MAX_Z = 64000;
const BLACK_SQUARES = [
	'in_lava_blacksquare', 't_aid_blackcircle_01', 't_aid_blackcircle_02', 't_aid_blacktriangle_01'
];
...
module.exports = {
	...
	onCellRef(record, reference, id) {
		...
		if(!('deleted' in reference) && !interior) {
			const [x, y] = record.data.grid;
		...
		if(interior && BLACK_SQUARES.includes(id) && hasWater(record)) {
			const [x, y] = reference.rotation;
			if(x === 0 && y === 0) {
				const cell = getCellName(record);
				const key = `${cell}_${id}`;
				if(!seen.has(key)) {
					seen.add(key);
					console.log(record.type, cell, 'contains black square', reference.id, 'despite having water');
				}
			}
		}
	}
