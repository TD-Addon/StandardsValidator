...
module.exports = {
	onRecord(record, currentTopic) {
		if(['Armor', 'Book', 'Clothing', 'Weapon'].includes(record.type)) {
			...
			record.inventory?.forEach(([/*count*/, item]) => check(item, record.id, record.type));
		} else if(record.type === 'Cell') {
			const name = record.id || record.region;
			record.references?.forEach(ref => {
				check(ref.id, name, record.type);
			});
			...
			record.creatures?.forEach(([item]) => check(item, record.id, record.type));
		} else if(record.type === 'LevelledItem') {
			record.items?.forEach(([item]) => check(item, record.id, record.type));
