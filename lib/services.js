	...
	onRecord(record, mode, recordId) {
		...
		if(isClass || ['Creature', 'Npc'].includes(record.type)) {
			if(barters(record)) {
				...
				} else if(record.data?.gold) {
					console.log(record.type, record.id, 'has barter gold but does not barter');
				}
				if(isClass && BARTER_CLASSES.has(recordId)) {
