...
const SPELLS = (() => {
	...
	const out = {};
	const rules = {};
	for(const id in spells.races) {
		const prefix = spells.races[id].prefix?.toLowerCase();
		const race = spells.races[id].race?.toLowerCase();
		rules[id] = (record, recordId) => {
			if(prefix && recordId.startsWith(prefix)) {
				return true;
			}
			return record.race?.toLowerCase() === race;
		};
	}
	spells.alternatives.forEach(alternatives => {
		const ids = Object.values(alternatives).map(id => id.toLowerCase());
		for(const rule in alternatives) {
			out[alternatives[rule].toLowerCase()] = (record, recordId) => {
				const matches = rules[rule](record, recordId);
				return [matches, ids];
			};
		}
	});
	const never = () => [false, []];
	spells.blacklist.forEach(id => {
		out[id.toLowerCase()] = never;
	})
	return out;
})();
...
module.exports = {
	onRecord(record, mode, recordId) {
		if(record.type === 'Npc' && !isDead(record)) {
			record.spells?.forEach(spell => {
				const id = spell.toLowerCase();
				if(id in SPELLS) {
					const [valid, alternatives] = SPELLS[id](record, recordId);
					if(!valid) {
						const matches = alternatives.filter(alt => SPELLS[alt](record, recordId)[0]);
						if(matches.length) {
							console.log(record.type, record.id, 'knows spell', spell, 'which should probably be', matches.join(' or '));
						} else {
							console.log(record.type, record.id, 'knows spell', spell);
						}
					}
				}
			});
		} else if(['Alchemy', 'Enchantment', 'Spell'].includes(record.type)) {
