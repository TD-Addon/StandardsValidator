...
module.exports = {
	onRecord(record, topic, mode) {
		...
		if(record.type === 'Script') {
			...
			record.references.forEach(reference => {
				OBJECT_IDS.delete(reference.id.toLowerCase());
			});
		} else {
			if(record.script) {
	...
	onEnd() {
		...
		OBJECT_IDS.forEach(id => {
			console.log('Object', id, 'is not used');
		});
	}
};
