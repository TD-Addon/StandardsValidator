...
module.exports = {
	...
	onScriptLine(record, line) {
		...
		const first = /^([,\s]*|.*?->[,\s]*)(placeatme|addsoulgem|additem|equip|drop|placeatpc|placeitemcell|placeitem|journal|setjournalindex)[,\s]+(?:("[^"]+"?)(?:.*)|([^,\s"]+)(?:[,\s]+|$))/.exec(line);
		if(first) {
			const [, , command, quotedId, unquotedId] = first;
			let id = unquotedId;
			if(quotedId) {
				id = quotedId.replace(/"/g, '');
			}
			if(command === 'journal' || command === 'setjournalindex') {
				return USED_JOURNALS.add(id);
			}
