...
module.exports = {
	onRecord(record) {
		if(record.type === 'Cell') {
			...
			const { length } = record.references;
			record.references.forEach((reference, start) => {
				...
				for(let i = start + 1; i < length; i++) {
					...
					if(other.id.toLowerCase() === id && equals(reference.rotation, other.rotation) && (reference.scale ?? 1) === (other.scale ?? 1) && translation(reference.translation, other.translation)) {
						console.log(record.type, name, 'contains duplicate reference', reference.id, 'at position', reference.translation, other.translation);
					}
				}
			});
		}
