...
module.exports = {
	...
	onInfo(record, topic) {
		if(record.speaker_id && /^destination$/i.test(topic.id)) {
			...
			if(id in CARAVANERS) {
				CARAVANERS[id].destination = record;
			}
		}
	},
	onEnd() {
			...
			cells.forEach(cell => {
				...
				record.travel_destinations.forEach(dest => {
					...
					if(!returnServices.length) {
						console.log(record.type, record.id, 'in', getCellName(cell.record), 'offers travel to', getDestinationName(dest), 'but there is no return travel there');
					} else if(record.class && !returnServices.some(counterpart => !counterpart.record.class || record.class === counterpart.record.class)) {
						console.log(record.type, record.id, 'in', getCellName(cell.record), 'offers', record.class, 'travel to', getDestinationName(dest), 'but there is no corresponding return travel');
					}
				});
			});
		});
