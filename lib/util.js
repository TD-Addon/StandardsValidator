...
const FLAG_CELL_WATER = 2;
const FLAG_CELL_NO_SLEEP = 4;
const FLAG_KEY = 1;
...
const ITEMS = ['Alchemy', 'Apparatus', 'Armor', 'Book', 'Clothing', 'Ingredient',
	'Lockpick', 'MiscItem', 'Probe', 'RepairTool', 'Weapon'];
...
function hasWater(record) {
	return !isInterior(record) || Boolean(record.type === 'Cell' && record.data?.flags & FLAG_CELL_WATER);
}

function cannotSleep(record) {
	return Boolean(record.type === 'Cell' && record.data?.flags & FLAG_CELL_NO_SLEEP);
}
...
function isItem(record) {
	return ITEMS.includes(record.type) || canCarry(record);
}

function isFemale(record) {
	if(record.type === 'Npc') {
		return Boolean(record.npc_flags & FLAG_FEMALE);
	} else if(record.type === 'Bodypart') {
		return Boolean(record.data?.female & FLAG_FEMALE);
	}
	return false;
}
...
module.exports = {
	...
	getCellGrid, canCarry, cannotSleep, hasWater, isItem, isFemale
};
