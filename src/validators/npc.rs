...
use super::Context;
use crate::{context::Mode, handlers::Handler, util::is_dead};
...
impl Handler<'_> for NpcValidator {
    fn on_record(&mut self, context: &Context, record: &TES3Object, _: &'static str, _: &String) {
        ...
        if let TES3Object::Npc(npc) = record {
            ...
            if context.mode == Mode::PT && (npc.npc_flags.unwrap_or(0) & FLAG_NPC_AUTO_CALC) != 0 {
...
impl RulesParser {
                    ...
                    if key == "class" {
                        ...
                        if let Some((part, model)) = params {
                            if let Rule::Equality(id) = &built {
                                self.uniques.entry(id.to_ascii_lowercase()).and_modify(|u| {
    ...
    fn parse_part(&mut self, definitions: &Vec<Definition>, part: BodypartId) {
    ...
    fn parse_rulesets(&mut self, rulesets: &HashMap<String, Vec<HashMap<String, Value>>>) {
